...
__global__ void GPU_HoughTranShared(unsigned char *pic, int w, int h, int *acc, float rMax, float rScale)
{
    ...
    // Acumulador local en memoria compartida
    __shared__ int localAcc[degreeBins * rBins];
    if (locID < degreeBins * rBins)
        localAcc[locID] = 0;
    __syncthreads(); // Barrera de sincronización para inicialización
    ...
    if (pic[gloID] > 0)
    {
        for (int tIdx = 0; tIdx < degreeBins; tIdx++)
        {
            float r = xCoord * dCos[tIdx] + yCoord * dSin[tIdx];
            int rIdx = (r + rMax) / rScale;
            atomicAdd(&localAcc[rIdx * degreeBins + tIdx], 1); // Suma atómica en localAcc
        }
    }
    __syncthreads(); // Barrera de sincronización después de incremento

    // Sumar los valores del acumulador local al global
